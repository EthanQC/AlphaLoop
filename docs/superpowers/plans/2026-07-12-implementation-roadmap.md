...
| 阶段 | 内容 | 交付判定 | 计划文件 |
|---|---|---|---|
| **P1 地基** | 版本化 DB 迁移 + 多成员表/owner 列 + 分散 DDL 清理 + 每日备份 + 失败停机状态机 + 飞书交互卡片能力 + 仓库卫生/宪法同步 | `pnpm test` 全绿；迁移可从现库无损升级；卡片 API 带 stub 单测 | `2026-07-12-phase1-foundation.md`（已写） |
| P2 提醒引擎 | market-alerts 规则评估（四类/滞回/冷却/配额/owner 隔离）+ CLI + launchd 轮询模板 + 交易日历 fail-loud | 全部规则单测；历史行情回放测试 | 待写 |
