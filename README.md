...
安装本地 launchd 调度——下面两条命令覆盖的任务集合完全不重叠，一台正式部署机通常两条都要跑：
...
只跑其中一条会漏装另一半任务——例如只跑 `launchd:install-user` 的机器不会有每日备份，也不会有盘中提醒器（`openclaw:runtime:doctor` 的 `launchd-jobs.*.not_loaded` 提示就是在检测这种情况）。
