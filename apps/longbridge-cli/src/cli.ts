...
// Order types whose REQUIRED companion parameters (triggerPrice for LIT/MIT,
// trailingAmount/trailingPercent/limitOffset for the trailing family) have no
// CLI flag yet. Rejected at the parser (exit 2, fail-fast) instead of letting
// the broker reject them server-side (exit 1, after a network round-trip).
const UNSUPPORTED_ORDER_TYPES = new Set(["LIT", "MIT", "TSLPAMT", "TSLPPCT", "TSMAMT", "TSMPCT"]);
...
function parseOrderCommand(tokens: TokenizedArgv): Command {
  ...
  if (sub === "buy" || sub === "sell") {
    ...
    // Canonical positive integer only: the raw string is forwarded verbatim
    // to the SDK's Decimal AND echoed in the submit payload, so exotic
    // spellings Number() would accept ("1e3", "1.5", "007", "0x10") must be
    // rejected here — otherwise the broker-submitted value and the echoed
    // quantity string can diverge for reconcile scripts.
    if (!/^[1-9]\d*$/u.test(quantityRaw)) {
      throw new UsageError(`数量必须是正整数（十进制、不带小数/指数/前导零），收到: ${quantityRaw}`);
    }
    ...
    if (UNSUPPORTED_ORDER_TYPES.has(orderType)) {
      throw new UsageError(
        `订单类型 ${orderType} 需要触发价/跟踪参数（--trigger-price / --trailing-* 等），本 CLI 尚未支持`
      );
    }
    ...
    if (timeInForce === "GoodTilDate") {
      throw new UsageError("有效期类型 GoodTilDate 需要 --expire-date 指定到期日，本 CLI 尚未支持");
    }

    const priceRaw = lastFlag(tokens, "--price");
    if (priceRaw !== undefined) {
      // Same canonical-form rule as quantity: plain decimal, no exponent /
      // hex / sign / leading zeros, and strictly positive.
      if (!/^(?:0|[1-9]\d*)(?:\.\d+)?$/u.test(priceRaw) || Number(priceRaw) <= 0) {
        throw new UsageError(`价格必须是正的十进制数（如 551.64），收到: ${priceRaw}`);
      }
...
命令:
  ...
  order <buy|sell> <SYMBOL> <QTY> --price <X.XX> [--order-type LO] [--tif Day]
        [--remark <文本>] [--outside-rth RTHOnly|AnyTime|Overnight] --yes
                                          下单（必须显式 --yes，本 CLI 不做交互确认）。
                                          QTY 必须是正整数，--price 必须是正的十进制数（不接受指数/小数股）；
                                          --order-type 仅支持 LO/ELO/MO/AO/ALO/ODD/SLO，--tif 仅支持 Day/GTC
                                          （LIT/MIT/跟踪单与 GTD 所需的 --trigger-price/--trailing-*/--expire-date
                                          尚未实现，会在解析阶段直接拒绝）；
                                          remark 超过 64 字符（LongPort OpenAPI 上限）时保留末尾 64 字符，
                                          确保结尾的工单 id 完整可检索
