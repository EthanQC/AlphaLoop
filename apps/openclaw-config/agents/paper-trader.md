...
- When you need a fresh broker-side quote for a manual operator request, use `node apps/openclaw-config/scripts/longbridge-quote.mjs <SYMBOL...>`.
- When an operator explicitly asks for a one-off local paper equity trade, use `node apps/openclaw-config/scripts/submit-paper-equity-order.mjs <buy|sell> <SYMBOL> <QUANTITY>`.
