// data/feishu-review-notifier.ts: the REAL review-confirm Feishu notifier
...
// the pure 月度复盘确认摘要 card composer. Mirrored, assertion for assertion,
// by apps/openclaw-config/scripts/feishu-review-notifier.test.ts (the .mjs
// sibling's suite) so the two faces cannot drift silently.
import { DatabaseSync } from "node:sqlite";

import { describe, expect, it } from "vitest";

import { MemberRepository, migrate, type CardTransport, type Member } from "@packages/shared-types";

import { composeReviewConfirmCardLines, createFeishuReviewNotifier } from "./feishu-review-notifier.js";
...
// numbers as routes/review.test.ts's fullResultFixture.
...
describe("composeReviewConfirmCardLines", () => {
...
describe("createFeishuReviewNotifier", () => {
  it("sends the interactive card to the member's feishu_open_id and reports {ok, messageId}", async () => {
    ...
    expect(payload.body.elements.map((element) => element.tag)).toEqual(lines.map(() => "markdown"));
